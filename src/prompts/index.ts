...
export function generateFileMap(
  ...
  for (const rootTreeItem of fullTreeRoots) {
    ...
    if (isRootSelectedOrHasSelectedDescendants) {
      // Display workspace root name clearly for multi-root workspace context
      lines.push(`[WORKSPACE ROOT: ${rootTreeItem.label}]`);
      ...
      lines.push(""); // Add a blank line between root sections for readability
    }
  }
